...
原神 (Genshin Impact) のUGC関連ファイルを .gil / .gia / .mihoyobin ⇔ .json 形式で相互変換するコンソールアプリです。

## 使い方
UgcFileConverter.exe [オプション] 入力ファイル名  
  
オプション一覧  
-d デコードモード： .gil / .gia / .mihoyobin → .json に変換  
-e エンコードモード： .json → .gil / .gia / .mihoyobin に変換  
-o 出力ファイル名: 出力ファイル名を指定  
-t dtypeファイル名: dtypeファイルを指定  
